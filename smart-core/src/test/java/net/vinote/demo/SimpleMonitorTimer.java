...
public class SimpleMonitorTimer extends QuickTimerTask implements SmartFilter<String> {
    ...
    public void processFilter(AioSession<String> session, String d) {
        processMsgNum.incrementAndGet();
        messageStorage.decrementAndGet();
        totleProcessMsgNum++;
    }

    public void readFilter(AioSession<String> session, String d, int readSize) {
        flow.addAndGet(readSize);
        recMsgnum.incrementAndGet();
        messageStorage.incrementAndGet();
    }

    public void processFailHandler(AioSession<String> session, String d) {
